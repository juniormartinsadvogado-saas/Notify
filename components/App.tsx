...
const App: React.FC = () => {
  ...
  return (
    <div className={`flex min-h-screen ${darkMode ? 'bg-slate-900 text-slate-100' : 'bg-zinc-100 text-slate-900'} overflow-x-hidden transition-colors duration-500`}>
      <Sidebar currentView={currentView} onChangeView={setCurrentView} onLogout={handleLogout} isOpen={isSidebarOpen} onToggle={() => setIsSidebarOpen(!isSidebarOpen)} badgeCounts={badgeCounts} />
